...
def test_load_sentiment_dataset_maps_source_repo_numeric_labels(tmp_path):
    df = pd.DataFrame(
        {
            "title": [
                "A good article",
    ...
    bundle = load_sentiment_dataset(DatasetLoadConfig(dataset_path=csv_path))
...
def test_load_sentiment_dataset_uses_filename_splits_when_available(tmp_path):
    ...
    bundle = load_sentiment_dataset(DatasetLoadConfig(dataset_path=tmp_path))
...
def test_load_sentiment_dataset_skips_unlabeled_auxiliary_files(tmp_path):
