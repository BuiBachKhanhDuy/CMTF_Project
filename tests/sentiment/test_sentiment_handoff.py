...
from src.sentiment.handoff import build_phobert_handoff, resolve_phobert_handoff, save_phobert_handoff
...
def test_sentiment_phobert_handoff_builds_and_saves(tmp_path):
    output_dir = tmp_path / "sentiment_latest"
    ...
    handoff = build_phobert_handoff(output_dir)
    handoff_path = save_phobert_handoff(output_dir)
    resolved = resolve_phobert_handoff(output_dir)
...
def test_sentiment_phobert_inference_bundle_loads_and_scores_titles(tmp_path, monkeypatch):
    output_dir = tmp_path / "sentiment_latest"
    ...
    bundle = load_phobert_inference_bundle(output_dir, device="cpu")
    inferencer = PhoBERTInferencer(bundle)
...
def test_sentiment_phobert_inference_bundle_falls_back_without_java(tmp_path, monkeypatch):
    output_dir = tmp_path / "sentiment_latest"
    tokenizer_dir = output_dir / "phobert" / "tokenizer"
    tokenizer_dir.mkdir(parents=True)
    checkpoint_path = output_dir / "phobert" / "phobert.pt"
    ...
    bundle = load_phobert_inference_bundle(
        output_dir,
        device="cpu",
