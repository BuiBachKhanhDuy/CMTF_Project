...
from src.multiagent.news_data import load_news_index, recent_headlines, articles_in_range
...
def _real_vol_dd(frame, symbol, cutoff, window=20):
    ...
    curve = np.exp(np.cumsum(daily)); peak = np.maximum.accumulate(curve)
    dd = float(((peak - curve) / peak).max() * 100)
    trend = float((np.exp(np.sum(daily)) - 1.0) * 100)  # compounded log return over the window
    return vol, dd, trend
...
def _extract_horizon(query, cfg, default_horizon, parsed=None):
    ...
    """
    out = parsed if parsed is not None else orchestrator_node({"query_text": query, "node_timings": {}}, cfg)
    mentions_horizon = any(
        k in query.lower() for k in
        ("1d", "5d", "20d", "ngày", "tuần", "tháng", "day", "week", "month",
         "dài hạn", "trung hạn", "ngắn hạn")
    )
    return _HORIZON_DAYS.get(out.get("target_horizon", ""), default_horizon) if mentions_horizon else default_horizon
...
def _run_rank(symbols, date, horizon, cfg):
    out = rank_agent_node({"target_symbols": symbols, "target_horizon_days": horizon,
                           "prediction_time": date, "node_timings": {}}, cfg)
    print(f"\n  RANKING @ {date}:")
...
def _run_prediction_for_gap(symbol, from_date, horizon, cfg, frame, news_idx):
    ...
    for _, fn in _DECISION_ONLY_CHAIN:  # predict, gate, horizon_interaction, risk, metalabel — no narrator/critic needed here
...
_NEWS_THEMES = {
    ...
    "lãi suất / chính sách tiền tệ": ("lãi suất", "ngân hàng nhà nước", "sbv", "tiền tệ", "tín dụng"),
    "VN-Index / thị trường chung": ("vn-index", "vnindex", "chứng khoán", "cổ phiếu", "thị trường"),
    "Fed / vĩ mô quốc tế": ("fed", "powell", "mỹ", "gdp", "lạm phát"),
    "nội tại VCB / ngân hàng": ("vietcombank", "vcb", "lợi nhuận", "cổ tức", "chi nhánh"),
}
...
def _deterministic_conclusion(symbol, date_start, date_end, stats, articles, prediction) -> str:
    ...
    if stats["coverage"] == "none":
        parts.append(f"Không có dữ liệu giá thật của {symbol} trong khoảng {date_start}..{date_end}.")
    else:
        r = stats["return_pct"]; v = stats["volatility_pct"]; dd = stats["max_drawdown_pct"]
    ...
    if themes:
        ...
        parts.append(f"Dòng tin ({len(articles)} bài, trải đều khoảng) tập trung vào: {top}.")
    if prediction is not None:
        act = {"long": "khả năng tăng", "short": "khả năng giảm", "abstain": "không đủ độ tin cậy để khuyến nghị"}.get(
            prediction.get("action"), prediction.get("action"))
        parts.append(
...
def _llm_synthesize_research(symbol, date_start, date_end, stats, articles, prediction, cfg):
    ...
    if articles:
        ...
        fact_lines += [f"  [{a['id']}] ({a['published_at']}) {a['title']}" for a in articles[:15]]
    else:
    ...
    return llm.invoke([("system", system), ("human", "\n".join(fact_lines))]).content.strip()
...
def _run_research(symbol, date_start, date_end, horizon, cfg, frame, news_idx):
    ...
    """
    with _collecting(f"Đang thu thập dữ liệu thị trường & tin tức {symbol} ({date_start}..{date_end})…"):
        ...
        if stats["coverage"] in ("partial", "none"):
            ...
            prediction = _run_prediction_for_gap(symbol, anchor, horizon, cfg, frame, news_idx)

    # Compact factual header (real numbers only).
    print(f"\n  📊 {symbol}  {date_start} .. {date_end}  (dữ liệu thật: {stats['coverage']})")
    if stats["coverage"] != "none":
        print(f"     {stats['covered_start']}..{stats['covered_end']} ({stats['n_days']} ngày): "
    ...
    if _llm_reachable(cfg):
        try:
            llm_cfg = _dc_replace(cfg, evaluation_mode=False)  # research is narration, not the deterministic decision path
            with _collecting("Đang phân tích…"):
                answer = _llm_synthesize_research(symbol, date_start, date_end, stats, articles, prediction, llm_cfg)
            print(f"  💬 {answer}")
            return
        except Exception as e:  # noqa: BLE001 — surface, fall back, never hide (R1)
            print(f"  (LLM tạm không khả dụng: {type(e).__name__}; dùng phân tích xác định thay thế)")
    print(f"  💬 {_deterministic_conclusion(symbol, date_start, date_end, stats, articles, prediction)}")
...
def main():
    ...
    frame = pd.read_parquet(resolve_price_parquet(args.horizon, allow_missing_target=True))
    ...
    while True:
        ...
        if low in ("quit", "exit", "q"):
            print("bye."); break
        if low in ("help", "?"):
            print("  Type: <SYMBOL> [YYYY-MM-DD] [horizon]  |  natural question (EN/VI)  |  "
        ...
        if intent == "COMPARISON":
            if len(symbols) >= 2 and date:
                print(f"  → orchestrator: intent={intent} symbols={symbols} @ {date}  ({route_reason})")
                _run_rank(symbols, date, horizon, cfg)
                continue
            print(f"  → orchestrator: intent={intent}  ({route_reason})")
            print("  Comparison needs 2+ known symbols and a date, e.g. "
                  "'rank VCB,BID,CTG 2025-08-13' or 'VCB vs BID 2025-08-13'.")
            continue

        if intent == "RESEARCH":
            print(f"  → orchestrator: intent={intent} symbol={symbol} range={date_start}..{date_end}  ({route_reason})")
            try:
                _run_research(symbol, date_start, date_end, horizon, cfg, frame, news_idx)
