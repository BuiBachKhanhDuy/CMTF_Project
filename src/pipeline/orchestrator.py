...
def run_pipeline(config: dict[str, Any]) -> CMTFDataset:
    ...
    for symbol in symbols:
        ...
        try:
            ...
            if mode in {"vci", "api"}:
                ...
                if not df_news.empty:
                    ...
                    df_news = df_news[(df_news["published_date"] >= start_ts) & (df_news["published_date"] <= end_ts)]
            else:
        ...
        if log_news_coverage:
            ...
            logger.info("News coverage | {} | {} / {} bars ({:.2f}%)", symbol, bars_with_news, total_bars, pct)
    ...
    try:
        ...
        df_all = df_all.merge(vnidx_features, left_index=True, right_index=True, how="left")
    ...
    if log_news_coverage and "has_news" in df_all.columns:
        ...
        logger.info("Encoded news coverage | {} / {} rows ({:.2f}%) have non-zero news", has_news_count, total_rows, pct)

    # Market-only feature columns: exclude all sentiment/news-derived columns.
    market_feature_cols = [
...
def _compute_single_cutoff(
    ...
    *,
    ...
    if len(df_ohlcv) < sequence_len:
        raise ValueError(f"Not enough OHLCV bars for {symbol} up to {cutoff}: got {len(df_ohlcv)}, need {sequence_len}")
    ...
    df_news = pd.DataFrame(articles_raw) if articles_raw else pd.DataFrame(columns=["published_date", "title", "content"])
    ...
    try:
        ...
        vnidx_vol_ma = df_vnindex["volume"].rolling(window=20, min_periods=1).mean()
        vnidx_features["vnindex_vol_ratio"] = df_vnindex["volume"] / vnidx_vol_ma
        df_featured = df_featured.merge(vnidx_features, left_index=True, right_index=True, how="left")
        for col in ("vnindex_ret", "vnindex_vol_ratio"):
    ...
    # Pure market-only columns only
    market_cols = [
        c for c in _CANONICAL_MARKET_COLS
        if c in df_featured.columns and pd.api.types.is_numeric_dtype(df_featured[c])
    ]
    ...
    if len(df_valid) < sequence_len:
        raise ValueError(f"Not enough valid bars for {symbol} after TA warm-up: got {len(df_valid)}, need {sequence_len}")
