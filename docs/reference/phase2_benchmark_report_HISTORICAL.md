> **HISTORICAL SNAPSHOT — superseded.** This is a Phase-2 benchmark report from before the
> `unitstd_v2` correctness fix and the `anchored_fusion`/gate wiring. Numbers below reflect a
> superseded fusion methodology. For current results see
> [CMTF_FUSION_FINDINGS.md](CMTF_FUSION_FINDINGS.md) and [RESULTS_IMPROVEMENT_LEVERS.md](RESULTS_IMPROVEMENT_LEVERS.md).
> Kept for historical/audit trail only — do not cite these numbers in the thesis.
